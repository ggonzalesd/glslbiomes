...
vec3 calcPointLight(PointLight light, vec3 normal, vec3 viewDir){
	...
	vec3 ambient = light.ambient * texture(diffuseMap, v_texcoord).rgb;
	vec3 diffuse = light.diffuse * diff * texture(diffuseMap, v_texcoord).rgb;
	vec3 specular = light.specular * spec * texture(specularMap, v_texcoord).rgb;
