...
class Cam {
  constructor(pos, speed=2.5, canvas) {
    this.canvas = canvas;
    this.pos = vec3.clone(pos);
    this.f_pos = vec3.fromValues(0, 0, 0);
  ...
  loopCam(deltaTime){
    this.pitch += this.f_pitch*0.15;
    this.yaw += this.f_yaw*0.15;

    this.f_pitch *= 0.8;
    this.f_yaw *= 0.8;

    if(this.last_direction >= 0){
      const velocity = this.sign[this.last_direction] * this.speed * deltaTime;
      this.f_pos[0] += this.vec[this.last_direction][0] * velocity;
      this.f_pos[1] += this.vec[this.last_direction][1] * velocity;
      this.f_pos[2] += this.vec[this.last_direction][2] * velocity;
    }

    this.pos[0] += this.f_pos[0];
    this.pos[1] += this.f_pos[1];
    this.pos[2] += this.f_pos[2];
  ...
  processPov(xoffset, yoffset, constrainPitch) {
    ...
    this.f_yaw += xoffset * this.mouseSensitivity;
    this.f_pitch += yoffset * this.mouseSensitivity;
...
async function loadMaterialTextures(gl, material){
  const textures = {
  ...
  Object.entries(material)
    ...
    .forEach(([key, filename]) => {
      ...
      if (!texture) {
        texture = twgl.createTexture(gl, { src: filename, flipY: true });
...
function generateObj(gl, data, meshProgramInfo, material, transforms){
  const textures = {
    defaultWhite: twgl.createTexture(gl, { src: [255, 255, 255, 255] }),
  };
